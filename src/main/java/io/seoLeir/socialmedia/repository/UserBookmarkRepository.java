...
public interface UserBookmarkRepository extends JpaRepository<UserBookmark, UserBookmarksId>,
    ...
    @Query("select ub.id.publicationUuid from UserBookmark ub where ub.id.userUsername = :username " +
            "order by ub.bookmarkedDate desc")
    List<UUID> getAllPublicationsByUserUsername(@Param("username") String username);
